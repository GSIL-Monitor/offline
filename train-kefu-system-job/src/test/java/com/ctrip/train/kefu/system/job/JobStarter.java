...
public class JobStarter {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Desktop.getDesktop().browse(new URI(String.format("http://127.0.0.1:%s/healthcheck.html", port)));
  ...
  private static String getWorkingDir() throws Exception {
    URL url = JobStarter.class.getClassLoader().getResource(APP_PROPERTIES);
    if (url != null) {
  ...
  private static int getPort() throws Exception {
    ...
    try {
      prop.load(JobStarter.class.getClassLoader().getResourceAsStream(APPLICATION_PROPERTIES));
