...
<@header pageTtle="一次性解决率报表" headerVisible=true pageWidth=1200></@header>
    ...
    <div class="panel panel-default" style="padding: 5px 10px;margin: 10px 0">
        <table width="100%"  class="table-style2">
            <tr>
                ...
                <th>员工:</th>
                <td><input type="text" style="width: 100px" class="form-control input-sm" id="OpUser"/></td>
...
<#noparse >
    ...
    <tr style="border-bottom:0">
         ...
         <td>${allsovle}</td>
         <td>${percentage}</td>
         <td> <a id="btnExport" onclick="exportNoticeDetail('${opTime}','${opuserNum}','${envenType}','${productLine}')" >导出</a> </td>
    </tr>
</script>
...
<script type="text/javascript">
    ...
    function getUrl() {
        var url="${ctx}/notice/searchoncesovle.json";
        var startDate=$("#inputStartTime input").val();
        var endDate=$("#inputEndTime input").val();
        var productLine=$("#selProductLine").val();
        var eventType=$("#selEventType").val();
    ...
    function exportNoticeDetail(opTime,opUserNum,envenType,productLine) {
        var url="exportoncedetail";
